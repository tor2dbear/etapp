...
const CASES = [
  ...
  { gate: "lookups", claim: "…and a property path spelled with brackets",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\]`, \"gu\");",
      "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[zzzz\\\\s*(${CONSTANT})\\\\s*\\\\]`, \"gu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a key of a pattern that is written as a string",
    edit: ["scripts/check-lookups.mjs", "  const PROP = new RegExp(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");",
      "  const PROP = new RegExp(`(?:(${ID})|(zzzz)|\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a numeric step in a path",
    edit: ["scripts/check-lookups.mjs", "  const STEPS = `(?:\\\\s*(?:\\\\?\\\\.|\\\\.)\\\\s*${ID}|\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*(?:${CONSTANT})\\\\s*\\\\])`;",
  ...
  { gate: "lookups", claim: "…and a computed key written as a number",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = new RegExp(`\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");",
  ...
  { gate: "lookups", claim: "…and a computed constant key, which is a key",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = new RegExp(`\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");",
      "  const COMPUTED = new RegExp(`zzzz\\\\s*(${CONSTANT})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");"],
    expect: "[fixture]" },
