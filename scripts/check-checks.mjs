...
const CASES = [
  ...
  { gate: "lookups", claim: "\u2026and grouping parentheses around an inline literal",
    edit: ["scripts/check-lookups.mjs", "    if (end < 0 || constantKey(end - 1)) continue;", "    continue;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "\u2026and a function body inside an initializer, which is not a map",
    edit: ["scripts/check-lookups.mjs", "        if (m && (m[0] !== \"{\" || kinds.get(i) === \"literal\") && !(m[0] === \"[\" && selected)) {",
      "        if (m && !(m[0] === \"[\" && selected)) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a quoted key no identifier could spell",
    edit: ["scripts/check-lookups.mjs", "  const CONSTANT_KEY = pattern(`(${CONSTANT})\\\\s*:\\\\s*${VALUE}`, \"y\");",
      "  const CONSTANT_KEY = pattern(`(\"${ID}\")\\\\s*:\\\\s*${VALUE}`, \"y\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a property path spelled with brackets",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = pattern(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"g\");",
      "  const SEGMENT = pattern(`\\\\.\\\\s*(${ID})|\\\\[zzzz\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"g\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a name that is the whole initialiser, whatever it is wrapped in",
    edit: ["scripts/check-lookups.mjs", "    commit();\n    // The two kinds of candidate", "    // The two kinds of candidate"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and an operand that stops being a name after it has been read",
    edit: ["scripts/check-lookups.mjs", "      if (!poisoned) for (const cand of current) if (cand.path.length || !ALIAS_SKIP.has(cand.root)) list.push(cand);",
      "      for (const cand of current) if (cand.path.length || !ALIAS_SKIP.has(cand.root)) list.push(cand);"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and the alphabet JavaScript spells names in, which is not ASCII",
    edit: ["scripts/check-lookups.mjs", "const ID = `(?:[\\\\p{ID_Start}$_]|${ESCAPED})(?:[\\\\p{ID_Continue}$\\\\u200C\\\\u200D]|${ESCAPED})*`;",
      "const ID = `(?:[a-zA-Z$_]|${ESCAPED})(?:[a-zA-Z0-9$_]|${ESCAPED})*`;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and where a name begins, which an ASCII boundary cannot say",
    edit: ["scripts/check-lookups.mjs", "const EDGE = \"(?<![.\\\\p{ID_Continue}$])\";",
      "const EDGE = \"\\\\b\";"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a sign in front of one, which only counts before a digit",
    edit: ["scripts/check-lookups.mjs", "} else if (numberAt(i) || ((quote === \"-\" || quote === \"+\") && numberAt(i + 1))) {",
      "} else if (numberAt(i)) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a floor on what the analysis *reaches*, not only on what it counts",
    edit: ["scripts/check-lookups.mjs", "  const INDEXED = pattern(`${rooted(`(${ID})`)}${INDEX}`, \"g\");", "  const INDEXED = pattern(`zzzz${INDEX}`, \"g\");"],
    expect: "[coverage]" },
  { gate: "lookups", claim: "…and a shorthand property, which is its own value",
    edit: ["scripts/check-lookups.mjs", "  const KEY = pattern(`(${ID})\\\\s*(?::\\\\s*${VALUE}|(?=[,}]|$))`, \"y\");",
      "  const KEY = pattern(`(${ID})\\\\s*(?::\\\\s*${VALUE}|(?=zzzz))`, \"y\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a call that hands back what it was given, which is not a factory",
    edit: ["scripts/check-lookups.mjs", "        const through = c === \"(\" && !!throughAt(code.slice(Math.max(0, i - 40), i + 1));",
      "        const through = false;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and that only the first of such a call's arguments is its value",
    edit: ["scripts/check-lookups.mjs", "          if (group.through) {", "          if (false) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a name indexed on the spot through one",
    edit: ["scripts/check-lookups.mjs", "pattern(`(?:${GROUPED}|${THROUGH}\\\\s*)`, \"g\")", "pattern(GROUPED, \"g\")"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and that somebody else's call of that name is not one of them",
    edit: ["scripts/check-lookups.mjs", "const THROUGH = `${EDGE}(?:Object\\\\s*\\\\.\\\\s*(?:${THROUGH_CALLS.join(\"|\")})|(?:new\\\\s+)?Object)\\\\s*\\\\(`;",
      "const THROUGH = `(?:Object\\\\s*\\\\.\\\\s*(?:${THROUGH_CALLS.join(\"|\")})|(?:new\\\\s+)?Object)\\\\s*\\\\(`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and the flag that makes that lookbehind's alphabet an alphabet",
    edit: ["scripts/check-lookups.mjs", "const THROUGH_END = pattern(`${THROUGH}$`);", "const THROUGH_END = new RegExp(THROUGH + \"$\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and that the comma after one ends it",
    ...
    expect: "[fixture]" },
  // Round 39: a number may begin with its point, and an array that is selected from hands
  // back an element.
  { gate: "lookups", claim: "…and a key that begins with its decimal point",
    edit: ["scripts/check-lookups.mjs", "  const numberAt = (j) => /\\d/.test(code[j]) || (code[j] === \".\" && /\\d/.test(code[j + 1]));",
      "  const numberAt = (j) => /\\d/.test(code[j]);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one with a sign in front of that point",
    edit: ["scripts/check-lookups.mjs", "    } else if (numberAt(i) || ((quote === \"-\" || quote === \"+\") && numberAt(i + 1))) {",
      "    } else if (numberAt(i) || ((quote === \"-\" || quote === \"+\") && /\\d/.test(code[i + 1]))) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and an element selected out of an array literal",
    edit: ["scripts/check-lookups.mjs", "        selected = close >= 0 && indexAt(close + 1) >= 0;",
      "        selected = false;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that an array nobody indexes is still the array",
    edit: ["scripts/check-lookups.mjs", "        selected = close >= 0 && indexAt(close + 1) >= 0;",
      "        selected = close >= 0;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and every element of one, since any of them may be the one",
    edit: ["scripts/check-lookups.mjs", "          branch(!!group.selected);", "          branch(false);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and the index that did the selecting, which is no read of what it selected",
    edit: ["scripts/check-lookups.mjs", "          if (saved.selected) {\n            const after = indexAt(i + 1);\n            const close = after >= 0 ? restOfCall(after) : -1;\n            if (close >= 0) i = close;\n          }\n", ""],
    expect: "[fixture]" },
  // Round 40: an operand is a root and a path, and a pattern binds names to paths of the
  // value beside it.
  { gate: "lookups", claim: "…and a property path on the right of an initialiser",
    edit: ["scripts/check-lookups.mjs", "        if (current.length && !poisoned && (c === \".\" || c === \"[\" || (c === \"?\" && code[i + 1] === \".\"))) {",
      "        if (false) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and every name a destructuring binds",
    edit: ["scripts/check-lookups.mjs", "  for (const m of [...code.matchAll(PATTERN_AT)].filter(inCode)) {", "  for (const m of []) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a pattern with no keyword in front of it",
    edit: ["scripts/check-lookups.mjs", "`(?:\\\\b(?:var|let|const)\\\\s+|(?<![\\\\p{ID_Continue}$.)\\\\]])\\\\(\\\\s*)(?=[{[])`",
      "`(?:\\\\b(?:var|let|const)\\\\s+)(?=[{[])`"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a shorthand in a pattern, which is its key and its name at once",
    edit: ["scripts/check-lookups.mjs", "          out.push({ name: key, path: path && path.concat(key), at: i });\n", ""],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a pattern written inside a pattern, which is a path further in",
    edit: ["scripts/check-lookups.mjs", "        patternBinds(target, inner, out);", "        void inner;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a key of a pattern that is written as a string",
    edit: ["scripts/check-lookups.mjs", "  const PROP = pattern(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"y\");",
      "  const PROP = pattern(`(?:(${ID})|(zzzz)|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one written as a computed constant",
    edit: ["scripts/check-lookups.mjs", "  const PROP = pattern(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"y\");",
      "  const PROP = pattern(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(zzzz)\\\\s*\\\\])\\\\s*(:)?`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that a string in a pattern is read rather than stepped over",
    edit: ["scripts/check-lookups.mjs", "  const spaced = (i) => {\n    while (i < code.length && /\\s/.test(code[i])) i++;\n    return i;\n  };",
      "  const spaced = (i) => skip(i);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a rest element, which is a new object with a prototype",
    edit: ["scripts/check-lookups.mjs", "  for (const r of restBinds) {", "  for (const r of []) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and an array pattern beside an array literal",
    edit: ["scripts/check-lookups.mjs", "      ? readInitialiser(spaced(eq + 1) + 1, true)", "      ? { found: [], holds: [], paths: [] }"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a pattern whose value is itself a path",
    edit: ["scripts/check-lookups.mjs", "    const roots = [...read.holds.map((held) => ({ root: held, path: [] })), ...read.paths];",
      "    const roots = [...read.holds.map((held) => ({ root: held, path: [] }))];"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a path assigned onto a path",
    edit: ["scripts/check-lookups.mjs", "    for (const p of (memberPaths.get(full) || new Map()).values()) {", "    for (const p of []) {"],
    expect: "[fixture]" },
  // Round 41: a key may be written as a number, on both sides of the reading.
  { gate: "lookups", claim: "…and a numeric step in a path",
    edit: ["scripts/check-lookups.mjs", "  const STEPS = `(?:\\\\s*(?:\\\\?\\\\.|\\\\.)\\\\s*${ID}|\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*(?:${BRACKETED})\\\\s*\\\\])`;",
      "  const STEPS = `(?:\\\\s*(?:\\\\?\\\\.|\\\\.)\\\\s*${ID}|\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*(?:${STRING}|${TEMPLATE})\\\\s*\\\\])`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and the same step where the path is read back out",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = pattern(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"g\");",
      "  const SEGMENT = pattern(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${STRING}|${TEMPLATE})\\\\s*\\\\]`, \"g\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a key of a literal written as a number",
    edit: ["scripts/check-lookups.mjs", "      const re = quoted ? CONSTANT_KEY : c === \"[\" ? COMPUTED : /[-+.\\d]/.test(c) ? CONSTANT_KEY : KEY;",
      "      const re = quoted ? CONSTANT_KEY : c === \"[\" ? COMPUTED : KEY;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…however that number is spelled",
    edit: ["scripts/check-lookups.mjs", "  const NUMBER = `[-+]?(?:0[xXoObB][0-9a-fA-F_]+|(?:\\\\d[\\\\d_]*(?:\\\\.[\\\\d_]*)?|\\\\.[\\\\d_]+)(?:[eE][-+]?[\\\\d_]+)?)n?`;",
      "  const NUMBER = `\\\\d+`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that a numeric key is what the number denotes, not what it says",
    edit: ["scripts/check-lookups.mjs", "    return Number.isNaN(n) ? null : String(n);", "    return raw;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a computed key written as a number",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = pattern(`\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");",
      "  const COMPUTED = pattern(`\\\\[\\\\s*(${STRING}|${TEMPLATE})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a numeric key in a pattern",
    edit: ["scripts/check-lookups.mjs", "        key = m[1] !== undefined ? asName(m[1]) : asKey(m[2] !== undefined ? m[2] : m[3]);",
      "        key = m[1] !== undefined ? asName(m[1]) : unescape((m[2] !== undefined ? m[2] : m[3]).slice(1, -1));"],
    expect: "[fixture]" },
  // Round 42: a template with nothing in it is a constant, and the anonymous side reads two
  // more shapes — an array that is selected from, and however many parentheses are wrapped
  // round a literal.
  { gate: "lookups", claim: "…and a key written as a template with nothing in it",
    edit: ["scripts/check-lookups.mjs", "    if (quote === \"`\") {", "    if (quote === \"zzzz\") {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that one with something in it is not that",
    edit: ["scripts/check-lookups.mjs", "  const TEMPLATE = \"`(?:[^`\\\\\\\\$]|\\\\\\\\.|\\\\$(?!\\\\{))*`\";",
      "  const TEMPLATE = \"`(?:[^`\\\\\\\\]|\\\\\\\\.)*`\";"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a template as a path step",
    edit: ["scripts/check-lookups.mjs", "  const STEPS = `(?:\\\\s*(?:\\\\?\\\\.|\\\\.)\\\\s*${ID}|\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*(?:${BRACKETED})\\\\s*\\\\])`;",
      "  const STEPS = `(?:\\\\s*(?:\\\\?\\\\.|\\\\.)\\\\s*${ID}|\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*(?:${CONSTANT})\\\\s*\\\\])`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one as a computed key",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = pattern(`\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");",
      "  const COMPUTED = pattern(`\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one as a computed key in a pattern",
    edit: ["scripts/check-lookups.mjs", "  const PROP = pattern(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"y\");",
      "  const PROP = pattern(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${CONSTANT})\\\\s*\\\\])\\\\s*(:)?`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and more than one parenthesis around a literal indexed on the spot",
    edit: ["scripts/check-lookups.mjs", "`(?:(?:${GROUPED})+|${THROUGH}\\\\s*)(?=\\\\{)`", "`(?:${GROUPED}|${THROUGH}\\\\s*)(?=\\\\{)`"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that it is as many closers as there were openers",
    edit: ["scripts/check-lookups.mjs", "    for (let n = (m[0].match(/\\(/g) || []).length; n > 0 && i >= 0; n--) {\n      i = code[i] === \")\" ? skip(i + 1) : -1;\n    }",
      "    while (code[i] === \")\") i = skip(i + 1);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and an element selected out of an array with no name at all",
    edit: ["scripts/check-lookups.mjs", "  for (const m of [...code.matchAll(pattern(`(?<![\\\\p{ID_Continue}$)\\\\]])\\\\[`, \"g\"))].filter(inCode)) {",
      "  for (const m of []) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that a bracket after a name is an index and not an array",
    edit: ["scripts/check-lookups.mjs", "  for (const m of [...code.matchAll(pattern(`(?<![\\\\p{ID_Continue}$)\\\\]])\\\\[`, \"g\"))].filter(inCode)) {",
      "  for (const m of [...code.matchAll(pattern(`\\\\[`, \"g\"))].filter(inCode)) {"],
    expect: "[fixture]" },
  // Round 43: three more spellings of a constant key, and one conversion that must not go
  // through a double.
  { gate: "lookups", claim: "\u2026and a key written as a code point escape",
    edit: ["scripts/check-lookups.mjs", "const unescape = (raw) => raw.replace(/\\\\(\\r\\n|[\\n\\r\\u2028\\u2029]|u\\{[0-9a-fA-F]+\\}|u[0-9a-fA-F]{4}|x[0-9a-fA-F]{2}|[\\s\\S])/g, (all, what) => {",
      "const unescape = (raw) => raw.replace(/\\\\(\\r\\n|[\\n\\r\\u2028\\u2029]|u[0-9a-fA-F]{4}|x[0-9a-fA-F]{2}|[\\s\\S])/g, (all, what) => {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a BigInt key, which no double can carry",
    edit: ["scripts/check-lookups.mjs", "    if (text.endsWith(\"n\")) {",
      "    if (false) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a literal keyword as an index",
    edit: ["scripts/check-lookups.mjs", "  const WORD = \"(?:true|false|null)\";",
      "  const WORD = \"(?:zzzz)\";"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a literal keyword as a step, a computed key and a pattern's key",
    edit: ["scripts/check-lookups.mjs", "  const BRACKETED = `${CONSTANT}|${WORD}|${TEMPLATE}`;",
      "  const BRACKETED = `${CONSTANT}|${TEMPLATE}`;"],
    expect: "[fixture]" },
  // Round 44: three spellings that are the same thing to the language — a separator in an
  // exponent, a string carried across a line, and a name written with an escape.
  { gate: "lookups", claim: "\u2026and a separator inside an exponent",
    edit: ["scripts/check-lookups.mjs", "  const NUMBER = `[-+]?(?:0[xXoObB][0-9a-fA-F_]+|(?:\\\\d[\\\\d_]*(?:\\\\.[\\\\d_]*)?|\\\\.[\\\\d_]+)(?:[eE][-+]?[\\\\d_]+)?)n?`;",
      "  const NUMBER = `[-+]?(?:0[xXoObB][0-9a-fA-F_]+|(?:\\\\d[\\\\d_]*(?:\\\\.[\\\\d_]*)?|\\\\.[\\\\d_]+)(?:[eE][-+]?\\\\d+)?)n?`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a string that carries across a line",
    edit: ["scripts/check-lookups.mjs", "  const DQ = `(?:[^\"\\\\\\\\]|\\\\\\\\[\\\\s\\\\S])*`;",
      "  const DQ = `(?:[^\"\\\\\\\\]|\\\\\\\\.)*`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and that the continuation decodes to nothing",
    edit: ["scripts/check-lookups.mjs", "  if (/^(?:\\r\\n|[\\n\\r\\u2028\\u2029])$/.test(what)) return \"\";",
      "  if (/^(?:\\r\\n|[\\n\\r\\u2028\\u2029])$/.test(what)) return all;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a name with an escape in it, read as one name",
    edit: ["scripts/check-lookups.mjs", "const ID = `(?:[\\\\p{ID_Start}$_]|${ESCAPED})(?:[\\\\p{ID_Continue}$\\\\u200C\\\\u200D]|${ESCAPED})*`;",
      "const ID = `(?:[\\\\p{ID_Start}$_])(?:[\\\\p{ID_Continue}$\\\\u200C\\\\u200D])*`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and the character that escape stands for",
    edit: ["scripts/check-lookups.mjs", "const asName = (raw) => (raw.indexOf(\"\\\\\") < 0 ? raw : unescape(raw));",
      "const asName = (raw) => (raw.indexOf(\"\\\\\") < 0 ? raw : raw);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and the same name spelled with one where it is looked for",
    edit: ["scripts/check-lookups.mjs", "    escapedNames",
      "    false"],
    expect: "[fixture]" },
  // Round 45: `Object()` makes one and `Object(x)` hands x back, and every pattern in the
  // gate is built with the flag its fragments are written in.
  { gate: "lookups", claim: "\u2026and a call to Object with nothing in it, which makes one",
    edit: ["scripts/check-lookups.mjs", "  const FACTORIES = `${EDGE}(?:Object\\\\.fromEntries\\\\(|JSON\\\\.parse\\\\(|(?:new\\\\s+)?Object\\\\s*\\\\(\\\\s*\\\\)|Object\\\\.create\\\\(\\\\s*\\\\{)`;",
      "  const FACTORIES = `${EDGE}(?:Object\\\\.fromEntries\\\\(|JSON\\\\.parse\\\\(|Object\\\\.create\\\\(\\\\s*\\\\{)`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and a call to Object with something in it, which hands that back",
    edit: ["scripts/check-lookups.mjs", "const THROUGH = `${EDGE}(?:Object\\\\s*\\\\.\\\\s*(?:${THROUGH_CALLS.join(\"|\")})|(?:new\\\\s+)?Object)\\\\s*\\\\(`;",
      "const THROUGH = `${EDGE}(?:Object\\\\s*\\\\.\\\\s*(?:${THROUGH_CALLS.join(\"|\")}))\\\\s*\\\\(`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and the flag every pattern here is built with",
    edit: ["scripts/check-lookups.mjs", "const pattern = (source, flags = \"\") => new RegExp(source, `${flags}u`);",
      "const pattern = (source, flags = \"\") => new RegExp(source, flags);"],
    expect: "[fixture]" },
  // Round 46: an array is not a table and a path still goes through one.
  { gate: "lookups", claim: "\u2026and an array as a property's value",
    edit: ["scripts/check-lookups.mjs", "  const VALUE = `(?:${THROUGH}\\\\s*)?(table\\\\(|dict\\\\(\\\\)|\\\\{|\\\\[|${ID}|)`;",
      "  const VALUE = `(?:${THROUGH}\\\\s*)?(table\\\\(|dict\\\\(\\\\)|\\\\{|${ID}|)`;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and an array as what a name holds",
    edit: ["scripts/check-lookups.mjs", "  const OPENER = pattern(`table\\\\(|dict\\\\(\\\\)|${FACTORIES}|\\\\{|\\\\[`, \"y\");",
      "  const OPENER = pattern(`table\\\\(|dict\\\\(\\\\)|${FACTORIES}|\\\\{`, \"y\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and the element a numeric step lands on",
    edit: ["scripts/check-lookups.mjs", "        if (b.opens === \"[\") {",
      "        if (false) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and that a step which is not a number is no element",
    edit: ["scripts/check-lookups.mjs", "          if (String(Number(path[at])) !== path[at]) continue;",
      "          if (false) continue;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and an array reached through a key rather than a name",
    edit: ["scripts/check-lookups.mjs", "        if (isLiteral(k.opens) || k.opens === \"[\") {",
      "        if (isLiteral(k.opens)) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "\u2026and that a selection is the element and not the array",
    edit: ["scripts/check-lookups.mjs", "        if (m && (m[0] !== \"{\" || kinds.get(i) === \"literal\") && !(m[0] === \"[\" && selected)) {",
      "        if (m && (m[0] !== \"{\" || kinds.get(i) === \"literal\")) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a computed constant key, which is a key",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = pattern(`\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");",
      "  const COMPUTED = pattern(`zzzz\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"y\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and an alias it stops walking back from",
    edit: ["scripts/check-lookups.mjs", "    for (const held of spread(name, heldFrom)) {", "    for (const held of [name]) {"],
    expect: "[fixture]" },
