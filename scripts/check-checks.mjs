...
const CASES = [
  ...
  { gate: "lookups", claim: "…and a nested literal it stops calling bare",
    edit: ["scripts/check-lookups.mjs", "(?::\\\\s*(table\\\\(|dict\\\\(\\\\)|\\\\{|${ID}|)", "(?::\\\\s*(table\\\\(|dict\\\\(\\\\)|zzzz|${ID}|)"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a condition, which is never a parameter list however it ends",
    edit: ["scripts/check-lookups.mjs", "        if (holder && holder.braces.length && !CONTROL.has(head)) {", "        if (holder && holder.braces.length) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and only the braces written straight inside that list",
    edit: ["scripts/check-lookups.mjs", "      if (kind === \"literal\" && holder && nesting === holder.depth + 1 && (last === \"(\" || last === \",\")) {",
      "      if (kind === \"literal\" && holder) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and parentheses around what a name is bound to",
    edit: ["scripts/check-lookups.mjs", "        const grouping = c === \"(\" && before !== \")\" && before !== \"]\" && !IDENT_PART.test(before);",
  ...
  { gate: "lookups", claim: "…and a parenthesised expression, whose value the walk already knows",
    edit: ["scripts/check-lookups.mjs", "    const { holds, at } = readInitialiser(m.index + m[0].length, true);", "    const { holds, at } = { holds: [], at: 0 };"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a comma inside one, which is a sequence and not a declarator’s end",
    edit: ["scripts/check-lookups.mjs", "    const outer = inGroup ? [{ list: [], poisoned: false }] : [];", "    const outer = [];"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a method, which is a name followed by a parenthesis and not one",
    edit: ["scripts/check-lookups.mjs", "      const short = m[1] !== undefined && m[2] === undefined;", "      const short = m[1] !== undefined;"],
    expect: "[fixture]" },
