...
const CASES = [
  ...
  { gate: "lookups", claim: "…nor is an empty object literal written anywhere",
    ...
    expect: "[bare]" },
  { gate: "lookups", claim: "a table bound without a keyword in front of its name is a table",
    edit: ["app.js", "      boardAt = table({ el: from,", "      boardAt = { el: from,"],
    expect: "[bare-table]" },
  { gate: "lookups", claim: "…and so is the second declarator of a list",
    edit: ["app.js", "entry = table({ name: name })", "entry = { name: name }"],
    expect: "[bare-table]" },
  { gate: "lookups", claim: "…and the shape it looks for still matches the file",
    edit: ["scripts/check-lookups.mjs", "(table\\(\\{|\\{)", "(zzzz\\(\\{|\\{)"],
    expect: "[coverage]" },
