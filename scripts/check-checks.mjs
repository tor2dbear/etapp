...
const CASES = [
  ...
  { gate: "lookups", claim: "\u2026and either branch of a conditional initializer",
    edit: ["scripts/check-lookups.mjs", "      if (depth === 0 && (c === \";\" || c === \",\")) break;",
      "      if (depth === 0) break;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a label\u2019s colon, which it then reads as a property\u2019s",
    edit: ["scripts/check-lookups.mjs", "      last = pendingLabel || (last === \"w\" && statementPlace(identStart)) ? \":label\" : \":\";",
      "      last = \":\";"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a quoted key no identifier could spell",
    edit: ["scripts/check-lookups.mjs", "  const QUOTED = /\"((?:[^\"\\\\]|\\\\.)*)\"\\s*:\\s*(table\\(|dict\\(\\)|\\{|)|'((?:[^'\\\\]|\\\\.)*)'\\s*:\\s*(table\\(|dict\\(\\)|\\{|)/y;",
      "  const QUOTED = /\"([A-Za-z_$][\\w$]*)\"\\s*:\\s*(table\\(|dict\\(\\)|\\{|)|'((?:[^'\\\\]|\\\\.)*)'\\s*:\\s*(table\\(|dict\\(\\)|\\{|)/y;"],
    expect: "[fixture]" },
