...
const CASES = [
  ...
  { gate: "lookups", claim: "…and an optional chain, which is the same read",
    edit: ["scripts/check-lookups.mjs", "  const INDEX = `\\\\s*(?:\\\\?\\\\.)?\\\\s*\\\\[\\\\s*[^\"'\\\\]]`;",
      "  const INDEX = `\\\\s*\\\\[\\\\s*[^\"'\\\\]]`;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a function expression, whose body closes a value",
    edit: ["scripts/check-lookups.mjs", "      if (maker !== null) { kind = maker ? \"fnvalue\" : \"block\"; maker = null; }",
      "      if (maker !== null) { kind = \"block\"; maker = null; }"],
    expect: "[fixture]" },
