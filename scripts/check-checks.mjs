...
const CASES = [
  ...
  { gate: "lookups", claim: "…and an element selected out of an array literal",
    edit: ["scripts/check-lookups.mjs", "          selected = close >= 0 && indexAt(close + 1) >= 0;", "          selected = false;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that an array nobody indexes is still the array",
    edit: ["scripts/check-lookups.mjs", "          selected = close >= 0 && indexAt(close + 1) >= 0;", "          selected = close >= 0;"],
    expect: "[fixture]" },
