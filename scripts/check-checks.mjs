...
const CASES = [
  ...
  { gate: "lookups", claim: "…however that number is spelled",
    edit: ["scripts/check-lookups.mjs", "  const NUMBER = `[-+]?(?:0[xXoObB][0-9a-fA-F_]+|(?:\\\\d[\\\\d_]*(?:\\\\.[\\\\d_]*)?|\\\\.[\\\\d_]+)(?:[eE][-+]?\\\\d+)?)n?`;",
      "  const NUMBER = `\\\\d+`;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a numeric key in a pattern",
    edit: ["scripts/check-lookups.mjs", "        key = m[1] !== undefined ? m[1] : asKey(m[2] !== undefined ? m[2] : m[3]);",
      "        key = m[1] !== undefined ? m[1] : unescape((m[2] !== undefined ? m[2] : m[3]).slice(1, -1));"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "\u2026and a key written as a code point escape",
    edit: ["scripts/check-lookups.mjs", "const unescape = (raw) => raw.replace(/\\\\(u\\{[0-9a-fA-F]+\\}|u[0-9a-fA-F]{4}|x[0-9a-fA-F]{2}|.)/g, (all, what) => {",
