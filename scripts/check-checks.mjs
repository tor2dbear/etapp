...
const CASES = [
  ...
  { gate: "lookups", claim: "…and a name assigned onto a property, which is that path’s and not the root’s",
    edit: ["scripts/check-lookups.mjs", "    if (path) link(memberHolds, path, held[1]);", "    if (false) link(memberHolds, path, held[1]);"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and a namesake property the alias side must not conflate either",
    edit: ["scripts/check-lookups.mjs", "    if (path) link(memberHolds, path, held[1]);",
      "    if (path) { link(memberHolds, path, held[1]); link(holders, held[1], steps[steps.length - 1]); }"],
    expect: "[fixture]" },
