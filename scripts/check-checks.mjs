...
const CASES = [
  ...
  { gate: "lookups", claim: "…and a quoted key no identifier could spell",
    edit: ["scripts/check-lookups.mjs", "  const CONSTANT_KEY = new RegExp(`(${CONSTANT})\\\\s*:\\\\s*${VALUE}`, \"yu\");",
      "  const CONSTANT_KEY = new RegExp(`(\"${ID}\")\\\\s*:\\\\s*${VALUE}`, \"yu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a property path spelled with brackets",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"gu\");",
      "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[zzzz\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"gu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a key of a pattern that is written as a string",
    edit: ["scripts/check-lookups.mjs", "  const PROP = new RegExp(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");",
      "  const PROP = new RegExp(`(?:(${ID})|(zzzz)|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one written as a computed constant",
    edit: ["scripts/check-lookups.mjs", "  const PROP = new RegExp(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");",
      "  const PROP = new RegExp(`(?:(${ID})|(${CONSTANT})|\\\\[\\\\s*(zzzz)\\\\s*\\\\])\\\\s*(:)?`, \"yu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and the same step where the path is read back out",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]`, \"gu\");",
      "  const SEGMENT = new RegExp(`\\\\.\\\\s*(${ID})|\\\\[\\\\s*(${STRING}|${TEMPLATE})\\\\s*\\\\]`, \"gu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a computed key written as a number",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = new RegExp(`\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");",
      "  const COMPUTED = new RegExp(`\\\\[\\\\s*(${STRING}|${TEMPLATE})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and an element selected out of an array with no name at all",
    edit: ["scripts/check-lookups.mjs", "  for (const m of [...code.matchAll(new RegExp(`(?<![\\\\p{ID_Continue}$)\\\\]])\\\\[`, \"gu\"))].filter(inCode)) {",
      "  for (const m of []) {"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and that a bracket after a name is an index and not an array",
    edit: ["scripts/check-lookups.mjs", "  for (const m of [...code.matchAll(new RegExp(`(?<![\\\\p{ID_Continue}$)\\\\]])\\\\[`, \"gu\"))].filter(inCode)) {",
      "  for (const m of [...code.matchAll(new RegExp(`\\\\[`, \"gu\"))].filter(inCode)) {"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a computed constant key, which is a key",
    edit: ["scripts/check-lookups.mjs", "  const COMPUTED = new RegExp(`\\\\[\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");",
      "  const COMPUTED = new RegExp(`zzzz\\\\s*(${BRACKETED})\\\\s*\\\\]\\\\s*:\\\\s*${VALUE}`, \"yu\");"],
    expect: "[fixture]" },
