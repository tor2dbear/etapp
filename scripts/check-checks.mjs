...
const CASES = [
  ...
  { gate: "lookups", claim: "\u2026and a binding pattern, which is not an empty map",
    edit: ["scripts/check-lookups.mjs", "      if (paramsAt >= 0 && nesting === paramsAt + 1 && (last === \"(\" || last === \",\")) kind = \"block\";",
      "      if (false) kind = \"block\";"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "\u2026and which brace is a function's body, once it has parameters",
    edit: ["scripts/check-lookups.mjs", "      else if (maker !== null && nesting === makerAt + 1) { kind = maker ? \"fnvalue\" : \"block\"; maker = null; }",
      "      else if (maker !== null) { kind = maker ? \"fnvalue\" : \"block\"; maker = null; }"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "\u2026and a case label closed by a colon at its own depth",
    edit: ["scripts/check-lookups.mjs", "      const closesLabel = pendingLabel && nesting === labelDepth;",
      "      const closesLabel = pendingLabel;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and parentheses around what a name is bound to",
    edit: ["scripts/check-lookups.mjs", "        const grouping = c === \"(\" && !/[\\w$)\\]]/.test(before);",
  ...
  { gate: "lookups", claim: "…and grouping parentheses around what is indexed",
    edit: ["scripts/check-lookups.mjs", "  const GROUPED = `(?<![\\\\w$)\\\\]])\\\\(\\\\s*`;",
      "  const GROUPED = `(?<![\\\\w$)\\\\]])zzzz\\\\(\\\\s*`;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a function expression, whose body closes a value",
    edit: ["scripts/check-lookups.mjs", "      else if (maker !== null && nesting === makerAt + 1) { kind = maker ? \"fnvalue\" : \"block\"; maker = null; }\n",
      "      else if (maker !== null && nesting === makerAt + 1) { kind = \"block\"; maker = null; }\n"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a quoted key no identifier could spell",
    edit: ["scripts/check-lookups.mjs", "  const QUOTED = /\"((?:[^\"\\\\]|\\\\.)*)\"\\s*:\\s*(table\\(|dict\\(\\)|\\{|[A-Za-z_$][\\w$]*|)|'((?:[^'\\\\]|\\\\.)*)'\\s*:\\s*(table\\(|dict\\(\\)|\\{|[A-Za-z_$][\\w$]*|)/y;",
      "  const QUOTED = /\"([A-Za-z_$][\\w$]*)\"\\s*:\\s*(table\\(|dict\\(\\)|\\{|[A-Za-z_$][\\w$]*|)|'((?:[^'\\\\]|\\\\.)*)'\\s*:\\s*(table\\(|dict\\(\\)|\\{|[A-Za-z_$][\\w$]*|)/y;"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a property path spelled with brackets",
    edit: ["scripts/check-lookups.mjs", "  const SEGMENT = new RegExp(`\\\\.\\\\s*([A-Za-z_$][\\\\w$]*)|\\\\[\\\\s*(${STRING})\\\\s*\\\\]`, \"g\");",
      "  const SEGMENT = new RegExp(`\\\\.\\\\s*([A-Za-z_$][\\\\w$]*)|\\\\[zzzz\\\\s*(${STRING})\\\\s*\\\\]`, \"g\");"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a receiver it stops keeping when the path’s root is not a name",
    edit: ["scripts/check-lookups.mjs", "`(?<![\\\\w$.)\\\\]])(?:\\\\b(?:var|let|const)", "`(?:\\\\b(?:var|let|const)"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and a name that is the whole initialiser, whatever it is wrapped in",
    edit: ["scripts/check-lookups.mjs", "    branch(true);\n    return { found, holds };", "    return { found, holds };"],
    expect: "[fixture]" },
  ...
  { gate: "lookups", claim: "…and an index, which is not the map it reads from",
    edit: ["scripts/check-lookups.mjs", "        if (!grouping && depth === 0) poisoned = true;", "        if (false) poisoned = true;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and an operand, which is not the value of the expression it is in",
    edit: ["scripts/check-lookups.mjs", "        else if (!/[([{)\\]}]/.test(c) && c !== \";\" && c !== \",\") poisoned = true;", "        else if (false) poisoned = true;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and the expression a parenthesis sits in, which it stops remembering",
    edit: ["scripts/check-lookups.mjs", "          poisoned = outer.pop() || poisoned;", "          poisoned = false;"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and the expression inside it, which a parenthesis does not make a name",
    edit: ["scripts/check-lookups.mjs", "          poisoned = outer.pop() || poisoned;", "          poisoned = outer.pop();"],
    expect: "[fixture]" },
