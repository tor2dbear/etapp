...
const CASES = [
  ...
  { gate: "lookups", claim: "…and a key of a pattern that is written as a string",
    edit: ["scripts/check-lookups.mjs", "  const PROP = new RegExp(`(?:(${ID})|(${STRING})|\\\\[\\\\s*(${STRING})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");",
      "  const PROP = new RegExp(`(?:(${ID})|(zzzz)|\\\\[\\\\s*(${STRING})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");"],
    expect: "[fixture]" },
  { gate: "lookups", claim: "…and one written as a computed constant",
    edit: ["scripts/check-lookups.mjs", "  const PROP = new RegExp(`(?:(${ID})|(${STRING})|\\\\[\\\\s*(${STRING})\\\\s*\\\\])\\\\s*(:)?`, \"yu\");",
      "  const PROP = new RegExp(`(?:(${ID})|(${STRING})|\\\\[\\\\s*(zzzz)\\\\s*\\\\])\\\\s*(:)?`, \"yu\");"],
    expect: "[fixture]" },
